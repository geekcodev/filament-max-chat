...
class ChatMessageServiceTest extends TestCase
{
    ...
    {
        $message = app(ChatMessageService::class)->storeIncoming($this->incomingUpdate('Привет!'));

        $this->assertNotNull($message);
        $this->assertDatabaseHas('chat_messages', [
            'id' => $message->id,
            'direction' => ChatMessageDirection::In->value,
            'sender_type' => ChatMessageSender::User->value,
            'text' => 'Привет!',
            'user_id' => 111,
            'chat_id' => 222,
        ]);

        $botChat = BotChat::query()->first();
        $this->assertNotNull($botChat);
        $this->assertSame(BotChatStatus::Active, $botChat->status);
        $this->assertNotNull($botChat->last_activity_at);
        $this->assertDatabaseHas('max_users', [
            'user_id' => 111,
            'first_name' => 'Иван',
        ]);
        $this->assertSame($botChat->id, $message->bot_chat_id);
    }
    ...
    public function test_store_outgoing_creates_out_message_with_operator(): void
    {
        ...
        $message = app(ChatMessageService::class)->storeOutgoing(
            userId: 111,
            chatId: 222,
            text: 'Добрый день!',
    ...
    public function test_conversations_returns_chats_with_unread_and_last_message(): void
    {
        ...
        $service->storeIncoming($this->incomingUpdate('Первое'));
        $service->storeIncoming($this->incomingUpdate('Второе'));
        $reply = $service->storeOutgoing(111, 222, 'Ответ', ChatMessageSender::Bot);
    ...
    public function test_conversations_excludes_non_active_chats(): void
    {
        ...
        BotChat::query()->firstOrFail()->update(['status' => BotChatStatus::Stopped]);
    ...
    public function test_messages_for_returns_chronological_messages(): void
    {
        $service = app(ChatMessageService::class);
        $service->storeIncoming($this->incomingUpdate('Первое'));
        $service->storeOutgoing(111, 222, 'Ответ', ChatMessageSender::Operator);
        $service->storeIncoming($this->incomingUpdate('Третье'));

        $botChat = BotChat::query()->firstOrFail();

        $messages = $service->messagesFor($botChat->id);
    ...
    public function test_mark_read_updates_only_incoming_unread(): void
    {
        ...
        $botChat = BotChat::query()->firstOrFail();
        $service->markRead($botChat->id);

        $this->assertSame(1, ChatMessage::query()->whereNull('read_at')->count());
        $incoming = ChatMessage::query()->where('direction', ChatMessageDirection::In)->firstOrFail();
    ...
    public function test_store_incoming_downloads_image_attachment(): void
    {
        ...
        $message = app(ChatMessageService::class)->storeIncoming($update);
    ...
    public function test_total_unread_count_excludes_read_messages(): void
    {
        ...
        $service->markRead($message->bot_chat_id);
    ...
    public function test_broadcast_with_includes_unread_count(): void
    {
        ...
        $this->assertSame($message->bot_chat_id, $data['bot_chat_id']);
    }
    ...
    public function test_messages_before_returns_older_messages(): void
    {
        ...
        $older = $service->messagesBefore($first->bot_chat_id, $second->id);
    ...
    public function test_messages_before_returns_empty_when_no_older(): void
    {
        ...
        $older = $service->messagesBefore($message->bot_chat_id, $message->id);
    ...
    public function test_delete_message_removes_from_db_and_max(): void
    {
        ...
        ChatMessage::query()->where('id', $message->id)->update(['message_id' => 'msg-123']);
    ...
    public function test_delete_message_still_removes_locally_when_max_fails(): void
    {
        ...
        ChatMessage::query()->where('id', $message->id)->update(['message_id' => 'msg-err']);
    ...
    public function test_clear_history_deletes_all_messages(): void
    {
        ...
        $chatMessage = ChatMessage::query()->first();
        $this->assertNotNull($chatMessage);

        ChatMessage::query()->update(['message_id' => 'msg-x']);

        $deleted = $service->clearHistory($chatMessage->bot_chat_id);
    ...
    public function test_clear_history_succeeds_when_max_delete_fails(): void
    {
        ...
        ChatMessage::query()->where('id', $message->id)->update(['message_id' => 'msg-fail']);

        $deleted = $service->clearHistory($message->bot_chat_id);
    ...
    public function test_clear_history_skips_messages_without_message_id(): void
    {
        ...
        ChatMessage::query()->where('id', $message->id)->update(['message_id' => null]);
